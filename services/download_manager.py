...
class DownloadManager:
    ...
    async def _download_via_torbox(self, task: DownloadTask, on_progress):
        ...
        # Encadenar resolver → TorBox: si la tarea arrastra un resolver, lo
        # corremos primero para obtener la URL directa (bypass de host/
        # Cloudflare) y se la damos a TorBox. Así TorBox baja cualquier host
        # que nuestros resolvers ya sepan convertir.
        src = task.url_or_magnet
        if task.resolver_callback is not None:
            try:
                resolved = await asyncio.to_thread(task.resolver_callback)
                if resolved:
                    src = resolved[0] if isinstance(resolved, tuple) else resolved
            except Exception:
                raise Exception(_("descarga.error_torbox_sin_link"))
